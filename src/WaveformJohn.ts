...
describe("Testing wave rendering basics", () => {
  var svg = d3
    .select<HTMLBodyElement, undefined>("body")
    .append<HTMLDivElement>("div")
    .append<SVGSVGElement>("svg");

  svg.attr("width", 500).attr("height", 500);

  var wave = new WaveGraph(
    svg as d3.Selection<SVGSVGElement, undefined, HTMLDivElement, undefined>,
  );
...
describe("Testing wave rendering of example", () => {
  ...
  var signalData = JSON.parse(
    fs.readFileSync("examples/FifoTC_test_normalOp.json", "utf8"),
  );
