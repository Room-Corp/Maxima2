/*
:note: Javascript version of https://github.com/Nic30/pyDigitalWaveTools/blob/master/pyDigitalWaveTools/vcd/parser.py
...
class VcdSyntaxError extends Error {}

class VcdDuplicatedVariableError extends Error {
  /*
...
function iNameScope_getDebugName(scope) {
  const buff = [];
  let o = scope;
  for (;;) {
    const n = o.name;
...
class VcdVarInfo {
  /*
    ...
    */
  constructor(vcdId, name, width, sigType, parent) {
    this.vcdId = vcdId;
    this.name = name;
    this.width = width;
  ...
  toString() {
    return `<${this.constructor.name} ${iNameScope_getDebugName(this)} vcdId:${this.vcdId}>`;
  }
}
...
class VcdVarScope {
  ...
  toString() {
    return `<${this.constructor.name} ${iNameScope_getDebugName(this)}>`;
  }
}

class VcdVarParsingInfo extends VcdVarInfo {
  /*
  ...
  toJson() {
    return {
      name: this.name,
      type: { width: this.width, name: this.sigType },
      data: this.data,
    };
  }
}
...
export class VcdParser {
  /*
    ...
    */
  constructor() {
    this.VECTOR_VALUE_CHANGE_PREFIX = new Set(["b", "B", "r", "R"]);
    this.SCOPE_TYPES = new Set(["begin", "fork", "function", "module", "task"]);
    ...
    this.keyword_dispatch = {
      // declaration_keyword ::=
      $comment: this.drop_while_end.bind(this),
      $date: this.save_declaration.bind(this),
      $enddefinitions: this.vcd_enddefinitions.bind(this),
      $scope: this.vcd_scope.bind(this),
      $timescale: this.save_declaration.bind(this),
      $upscope: this.vcd_upscope.bind(this),
      $var: this.vcd_var.bind(this),
      $version: this.save_declaration.bind(this),
      // simulation_keyword ::=
      $dumpall: this.vcd_dumpall.bind(this),
      $dumpoff: this.vcd_dumpoff.bind(this),
      $dumpon: this.vcd_dumpon.bind(this),
      $dumpvars: this.vcd_dumpvars.bind(this),
      $end: this.vcd_end.bind(this),
    };
  ...
  setNow(value) {
    if (Number.isFinite(value)) {
      ...
      this.now = parseInt(value); // TODO: can be float
    }
  }

  *createTokeniser(vcdStr) {
    let lineNo = 0;
  ...
  parse_str(vcdStr) {
    ...
    for (;;) {
      ...
      if (!fn) fn = this.parse_error;
      fn = fn.bind(this);
      fn(tokeniser, token.value[1]);
      if (this.end_of_definitions) break;
    }
  ...
  vcd_value_change(lineNo, token, tokenizer) {
    ...
    if (this.VECTOR_VALUE_CHANGE_PREFIX.has(token[0])) {
      ...
      // In many VCD files there is no $end terminator
      this.setNow(token.slice(1));
      return;
    } else {
      // 1 bit value
  ...
  drop_while_end(tokeniser) {
    dropwhile((x) => x[1] !== "$end", tokeniser);
  }
  *read_while_end(tokeniser) {
  ...
  vcd_scope(tokeniser, keyword) {
    ...
    if (scopeName.done) throw new VcdSyntaxError("Name of vcd scope expected");
  ...
  vcd_var(tokeniser, keyword) {
    ...
    let parent_var = this.idcode2var[vcdId];
    if (parent_var === undefined) parent_var = null;
    const info = new VcdVarParsingInfo(
      parent_var === null ? vcdId : parent_var,
      reference,
      size,
      var_type,
      parent,
  ...
  _vcd_value_change_list(tokeniser) {
    for (;;) {
      const item = tokeniser.next();
      if (item.done) {
        break;
      ...
      if (token && token[0] === "$") {
        if (token.startsWith("$end")) {
          ...
          throw new VcdSyntaxError(`Line ${lineNo}: Expected $end: ${token}`);
        }
      } else {
        this.vcd_value_change(lineNo, token, tokeniser);
      }
  ...
  vcd_dumpall(tokeniser, keyword) {
    /*
             ...
             .. code-block:: verilog$dumpall   1*@  x*#   0*$   bx   (k $end
                    */
